...
# from pybfl.transaction import *
from pybfl import OPCODE
...
class AddressClassTests(unittest.TestCase):
    ...
    def test_is_WIF_valid(self):
        # mainnet
        self.assertEqual(pybfl.PrivateKey("7B56E2B7BD189F4491D43A1D209E6268046DF1741F61B6397349D7AA54978E76",
        ...
        self.assertEqual(pybfl.PrivateKey("L1MU1jUjUwZ6Fd1L2HDZ8qH4oSWxct5boCQ4C87YvoSZbTW41hg4",
                                          compressed=False, testnet=True).wif,
                         'L1MU1jUjUwZ6Fd1L2HDZ8qH4oSWxct5boCQ4C87YvoSZbTW41hg4')

        p = pybfl.PrivateKey("L1MU1jUjUwZ6Fd1L2HDZ8qH4oSWxct5boCQ4C87YvoSZbTW41hg4")
        pub = pybfl.PublicKey(p)
        a = pybfl.Address(p)
        self.assertEqual(a.address, 'bc1qxsms4rt5axt9674du2az7vq3pvephu3k5jyky8')
        a = pybfl.Address(p, address_type ="P2PKH")
        self.assertEqual(a.address, '15m65JmFohJiioQbzMWhqFeCS3ZL1KVaNh')
        a = pybfl.Address(p, address_type ="PUBKEY")
        self.assertEqual(a.address, '15m65JmFohJiioQbzMWhqFeCS3ZL1KVaNh')
        a = pybfl.Address(p, address_type ="P2SH_P2WPKH")
        ...
        self.assertEqual(pybfl.PublicKey("7b56e2b7bd189f4491d43a1d209e6268046df1741f61b6397349d7aa54978e76",
        ...
        self.assertEqual(pybfl.PublicKey("L1MU1jUjUwZ6Fd1L2HDZ8qH4oSWxct5boCQ4C87YvoSZbTW41hg4",
                                         compressed=False).hex, cpk)
        # public key compressed from  PrivateKey instance (flags have no effect)
        p = pybfl.PrivateKey("L1MU1jUjUwZ6Fd1L2HDZ8qH4oSWxct5boCQ4C87YvoSZbTW41hg4")
        self.assertEqual(pybfl.PublicKey(p, compressed=False).hex, cpk)

        # public key compressed from  public
        self.assertEqual(pybfl.PublicKey(cpk, compressed=False).hex, cpk)

        # public key compressed from  public
        self.assertEqual(pybfl.PublicKey(unhexlify(cpk), compressed=False).hex, cpk)

        # compressed public key
        # private key hex string to compressed pub key
        p = pybfl.PrivateKey("7b56e2b7bd189f4491d43a1d209e6268046df1741f61b6397349d7aa54978e76", compressed=False)
        pub = pybfl.PublicKey(p)
        a = pybfl.Address(p, address_type="P2PKH")
        ...
        # from pubkey
        p = pybfl.PublicKey('02a8fb85e98c99b79150df12fde488639d8445c57babef83d53c66c1e5c818eeb4')
        a = pybfl.Address(p)
        self.assertEqual(a.address, 'bc1qxsms4rt5axt9674du2az7vq3pvephu3k5jyky8')
        a = pybfl.Address(p, address_type="P2PKH")
        self.assertEqual(a.address, '15m65JmFohJiioQbzMWhqFeCS3ZL1KVaNh')
        ...
        p = pybfl.PublicKey('04a8fb85e98c99b79150df12fde488639d8445c57babef83d53c66c1e5c818eeb43bbd96a641808'
        ...
        a = pybfl.ScriptAddress(redeem, witness_version=None)
